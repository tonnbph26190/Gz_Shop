...
namespace QuanApi.Controllers
{
    ...
    public class DanhMucsController : ControllerBase
    {
        ...
        public DanhMucsController(IDanhMucService service)
        {
            _service = service;
        }
        ...
        {
            var success = await _service.CreateAsync(dm);
        ...
        {
            var success = await _service.UpdateAsync(id, dm);
            return Ok(new { success });
        }
        ...
        public async Task<IActionResult> ToggleStatus(Guid id)
        {
            try
            {
                var currentStatus = await _service.ToggleStatusAsync(id);
                return Ok(new { success = true, trangThai = currentStatus });
            }
            catch (KeyNotFoundException)
            {
                return NotFound(new { success = false, message = "ID không tồn tại" });
            }
            catch (Exception ex)
            {
